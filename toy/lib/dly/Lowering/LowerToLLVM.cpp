...
// the input array. The file also sets up the DlyToLLVMLoweringPass. This pass
...
namespace {
struct DlyToLLVMLoweringPass
    : public PassWrapper<DlyToLLVMLoweringPass, OperationPass<ModuleOp>> {
  MLIR_DEFINE_EXPLICIT_INTERNAL_INLINE_TYPE_ID(DlyToLLVMLoweringPass)
...
void DlyToLLVMLoweringPass::runOnOperation() {
...
std::unique_ptr<mlir::Pass> mlir::toy::createLowerToLLVMPass() {
  return std::make_unique<DlyToLLVMLoweringPass>();
}
